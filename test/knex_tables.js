  ...
  [
    ...
    describe(dbTestValues.client, function() {
      beforeEach(function() {
        return BPromise.all([
          ...
          return dbTestValues.knex.raw(
            "CREATE VIEW test_view AS SELECT * FROM test_1"
          );
        });
      });
      ...
      it("can get all tables", function() {
        return knexTables
          .getTableNames(dbTestValues.knex)
          .then(function(tables) {
            tables.should.include(prefixTableName("test_1"));
            tables.should.include(prefixTableName("test_2"));
          });
      });
      ...
      it("can get all tables filtering by ignoreTables option", function() {
        return knexTables
          .getTableNames(dbTestValues.knex, {
            ignoreTables: [prefixTableName("test_1")]
          })
          .then(function(tables) {
            tables.should.not.include(prefixTableName("test_1"));
            tables.should.include(prefixTableName("test_2"));
          });
      });
      ...
      it("views should not be in the list of tables", function() {
        return knexTables
          .getTableNames(dbTestValues.knex)
          .then(function(tables) {
            tables.should.not.include(prefixTableName("test_view"));
          });
      });
    });
  });
